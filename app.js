...
function setupTransformPanel() {
  ...
  const setDD = (lat, lon) => { $("#tp-lat").value = lat.toFixed(6); $("#tp-lon").value = lon.toFixed(6); ddToUTM(); };
  ...
  const utmToDD = (zone, hem, E, N) => {
    const south = String(hem || "S").toUpperCase() === "S";
    const projUTM = `+proj=utm +zone=${zone} ${south ? "+south" : ""} +datum=WGS84 +units=m +no_defs +type=crs`;
    const ll = proj4(projUTM, "EPSG:4326", [E, N]);
    return { lat: ll[1], lon: ll[0] };
  };

  const ddToUTM = () => {
    const lat = parseNum($("#tp-lat").value), lon = parseNum($("#tp-lon").value);
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return;
    const p17 = proj4("EPSG:4326", EPSG32717, [lon, lat]);
    $("#tp-utm-e-17").value = p17[0].toFixed(2); $("#tp-utm-n-17").value = p17[1].toFixed(2);
  ...
  $("#tp-btn-center").addEventListener("click", () => {
    const lat = parseNum($("#tp-lat").value), lon = parseNum($("#tp-lon").value);
    if (Number.isFinite(lat) && Number.isFinite(lon)) map.setView([lat, lon], 15);
    else setStatus("Lat/Lon inválidos");
  });
  ...
  let fixedPin = null;
  (function rebindPinBtn(){
    const old = document.getElementById("tp-btn-pin");
    if (!old) return;
    const btn = old.cloneNode(true);
    old.replaceWith(btn);
    btn.addEventListener("click", () => {
      const lat = parseNum($("#tp-lat").value), lon = parseNum($("#tp-lon").value);
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) { setStatus("Lat/Lon inválidos"); return; }
      if (fixedPin) { map.removeLayer(fixedPin); fixedPin = null; }
      fixedPin = L.marker([lat, lon], { icon: crossIcon }).addTo(map);
    });
  })();

  ["tp-lat","tp-lon"].forEach(id=>{
    const el = document.getElementById(id);
    el.addEventListener("keydown", ev=>{
      if (ev.key === "Enter") {
        const lat = parseNum($("#tp-lat").value), lon = parseNum($("#tp-lon").value);
        if (Number.isFinite(lat) && Number.isFinite(lon)) { ddToUTM(); map.setView([lat, lon], 15); setStatus("Centrado por Lat/Lon"); }
      }
    });
  });

  function goToUTM(zone){
    const hem = (zone === 17 ? $("#tp-utm-hem-17") : $("#tp-utm-hem-18")).value || "S";
    const eEl = zone === 17 ? $("#tp-utm-e-17") : $("#tp-utm-e-18");
    const nEl = zone === 17 ? $("#tp-utm-n-17") : $("#tp-utm-n-18");
    const E = parseNum(eEl.value), N = parseNum(nEl.value);
    if (!Number.isFinite(E) || !Number.isFinite(N)) { setStatus("UTM: E/N inválidos"); return; }
    const { lat, lon } = utmToDD(zone, hem, E, N);
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) { setStatus("Conversión UTM→DD falló"); return; }
    setDD(lat, lon); map.setView([lat, lon], 15);
    if (fixedPin) { map.removeLayer(fixedPin); fixedPin = null; }
    fixedPin = L.marker([lat, lon], { icon: crossIcon }).addTo(map);
    setStatus(`Centrado en UTM ${zone}${hem}`);
  }

  [["tp-utm-e-17",17],["tp-utm-n-17",17],["tp-utm-e-18",18],["tp-utm-n-18",18]].forEach(([id,zone])=>{
    const el = document.getElementById(id);
    if (!el) return;
    el.addEventListener("keydown", ev=>{ if (ev.key === "Enter") { ev.preventDefault(); goToUTM(zone); } });
    el.addEventListener("blur", ()=>{
      const eVal = parseNum(zone===17?$("#tp-utm-e-17").value:$("#tp-utm-e-18").value);
      const nVal = parseNum(zone===17?$("#tp-utm-n-17").value:$("#tp-utm-n-18").value);
      if (Number.isFinite(eVal) && Number.isFinite(nVal)) goToUTM(zone);
    });
  });
