...
let map;
function initMap() {
  const esriLabels = L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Reference/World_Boundaries_and_Places/MapServer/tile/{z}/{y}/{x}", {maxZoom:19, attribution:"Labels & Boundaries © Esri"});
...
// Botón Fijar punto (si existe)
(function(){
  const btn = document.getElementById('tp-btn-pin');
  if (btn){
    btn.addEventListener('click', (e)=>{
      e.preventDefault();
      const latEl = document.querySelector('[data-field="latDD"]') || document.getElementById('latDD');
      const lonEl = document.querySelector('[data-field="lonDD"]') || document.getElementById('lonDD');
      const lat = parseFloat(latEl && latEl.value); const lon = parseFloat(lonEl && lonEl.value);
...
(function(){
  ...
  async function copyCoordsBlock() {
    ...
    try { await navigator.clipboard.writeText(payload); }
      ...
      const ta=document.createElement('textarea'); ta.value=payload; ta.style.position='fixed'; ta.style.opacity='0.001';
      document.body.appendChild(ta); ta.select(); try{ document.execCommand('copy'); } finally{ document.body.removeChild(ta); }
    }
  }
  const btn = document.getElementById('tp-btn-copy');
  if (btn){ btn.addEventListener('click', (e)=>{ e.preventDefault(); copyCoordsBlock(); }); }
})();
