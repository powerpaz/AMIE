...
(() => {
  ...
  function createPopupContent(inst) {
    ...
    const year2026 = (parseFloat(inst.MD_MONTO_USD) || 0) + (parseFloat(inst.M_MONTO_USD) || 0) + (parseFloat(inst.JE_MONTO_USD) || 0);
    const year2027 = year2026 * 0.45; // Ejemplo de cálculo para 2027
    const totalGeneral = year2026 + year2027;
        ...
        <div class="popup-body">
          ...
          <div class="popup-section">
            ...
            <div class="popup-info-row">
              ...
              <span class="popup-value">${inst.ZONA || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Provincia:</span>
              <span class="popup-value">${inst.PROVINCIA || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Cantón:</span>
              <span class="popup-value">${inst.CANTON || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Parroquia:</span>
              <span class="popup-value">${inst.PARROQUIA || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Distrito:</span>
              <span class="popup-value highlight">${inst.DISTRITO || 'N/A'}</span>
            </div>
          </div>
          ...
          <div class="popup-section">
            ...
            <div class="popup-info-row">
              ...
              <span class="popup-value">${inst.TIPO_EDUCACION || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Nivel:</span>
              <span class="popup-value">${inst.NIVEL_DE_EDUCACION || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Sostenimiento:</span>
              <span class="popup-value">${inst.SOSTENIMIENTO || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Jurisdicción:</span>
              <span class="popup-value">${inst.JURISDICCION || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Régimen:</span>
              <span class="popup-value">${inst.REGIMEN || 'N/A'}</span>
            </div>
            <div class="popup-info-row">
              <span class="popup-label">Estudiantes (3-4 años):</span>
              <span class="popup-value highlight">${inst.NRO_NINOS || 0}</span>
            </div>
          </div>`;

    // Sección de Presupuesto con nuevo diseño
    ...
    if (hasDotacion) {
      content += `
          ...
          <div class="popup-budget-section">
            <div class="popup-budget-title">💰 Presupuesto Asignado</div>
      ...
      if (inst.MD_MONTO_USD > 0) {
        content += `
            <div class="popup-info-row">
              <span class="popup-label">📚 Material Didáctico:</span>
              <span class="popup-money">${formatMoney(inst.MD_MONTO_USD)}</span>
            </div>`;
        if (inst.MD_PARALELOS) {
          content += `
            <div class="popup-info-row">
              <span class="popup-label" style="padding-left: 20px;">Paralelos:</span>
              <span class="popup-value">${inst.MD_PARALELOS}</span>
            </div>`;
        }
      }
      ...
      if (inst.M_MONTO_USD > 0) {
        content += `
            <div class="popup-info-row">
              <span class="popup-label">🪑 Mobiliario:</span>
              <span class="popup-money">${formatMoney(inst.M_MONTO_USD)}</span>
            </div>`;
        if (inst.M_TIPO_DE_AMBIENTE) {
          content += `
            <div class="popup-info-row">
              <span class="popup-label" style="padding-left: 20px;">Tipo Ambiente:</span>
              <span class="popup-value">${inst.M_TIPO_DE_AMBIENTE}</span>
            </div>`;
        }
      }
      ...
      if (inst.JE_MONTO_USD > 0) {
        content += `
            <div class="popup-info-row">
              <span class="popup-label">🎮 Juegos Exteriores:</span>
              <span class="popup-money">${formatMoney(inst.JE_MONTO_USD)}</span>
            </div>`;
        if (inst.JE_CANTIDAD) {
          content += `
            <div class="popup-info-row">
              <span class="popup-label" style="padding-left: 20px;">Cantidad:</span>
              <span class="popup-value">${inst.JE_CANTIDAD}</span>
            </div>`;
        }
      }
      ...
      content += `
            <div class="popup-budget-grid" style="margin-top: 15px;">
              <div class="popup-budget-item">
                <div class="popup-budget-year">2026</div>
                <div class="popup-budget-amount">${formatMoney(year2026)}</div>
              </div>
              <div class="popup-budget-item">
                <div class="popup-budget-year">2027</div>
                <div class="popup-budget-amount">${formatMoney(year2027)}</div>
              </div>
              <div class="popup-budget-item">
                <div class="popup-budget-year">Total</div>
                <div class="popup-budget-amount">${formatMoney(totalGeneral)}</div>
              </div>
            </div>
          </div>`;
      ...
      content += `
          <div class="popup-total">
            <div class="popup-total-label">Inversión Total Planificada</div>
            <div class="popup-total-amount">${formatMoney(totalGeneral)}</div>
          </div>`;
    } else {
      content += `
          <div class="popup-section">
            <h4>📊 Estado</h4>
            <div class="popup-info-row">
              <span class="popup-label">Dotación:</span>
              <span class="popup-badge warning">Sin asignación presupuestaria</span>
            </div>
          </div>`;
    }
    ...
    content += `
        ...
        <div class="popup-footer">
          <span class="popup-coordinates">📍 ${inst.LATITUD}, ${inst.LONGITUD}</span>
          <span class="popup-badge info">Actualizado 2025</span>
        </div>
      </div>`;
