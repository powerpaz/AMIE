...
(function() {
  ...
  function processData(rows) {
    return rows.map(row => {
      ...
      const nivelEducacion = row['NIVEL DE EDUCACIÓN'] || row['NIVEL_DE_EDUCACIÓN'] || row.NIVEL_DE_E || row.NIVEL_DE_EDUCACION || '';
      const anioDotacion = row['AUX_AÑO DE DOTACIÓN'] || row['AÑO_DE_DO'] || row.AUX_ANIO_DOTACION || row['AÑO DE DOTACIÓN'] || '';
      const presupuesto = row.Tipo_de_Pr || row.TIPO_DE_PR || row.TIPO_PRESUPUESTO || '';
      ...
      return {
        ...
        TIPO_EDUCACION: String(row.TIPO_EDUCA || row.TIPO_EDUCACION || '').trim(),
        SOSTENIMIENTO: String(row.SOSTENIMIE || row.SOSTENIMIENTO || '').trim(),
        JURISDICCION: String(row.JURISDICCI || row.JURISDICCION || '').trim(),
        ...
        NIVEL_DE_EDUCACION: String(nivelEducacion || '').trim(),
        AUX_ANIO_DOTACION: String(anioDotacion || '').trim(),
        TIPO_PRESUPUESTO: normalizeBudget(presupuesto),
        LATITUD: lat,
        LONGITUD: lng,
        _raw: row
      };
  ...
  function populateFilters() {
    ...
    populateSelect('nivelSel', uniqueSorted(data.map(r => r.NIVEL_DE_EDUCACION)), 'Todos los niveles');
    ...
    populateSelect('presupuestoSel', ['BANCO MUNDIAL', 'Fiscal'], 'Todos los tipos de presupuesto');
  }
  ...
  function setupEventListeners() {
    ...
    [
      ...
      { id: 'presupuestoSel', key: 'presupuesto' }
    ].forEach(({ id, key }) => {
      const el = document.getElementById(id);
      if (!el) return;
    ...
    setupDropdownButton('btnPresupuestoFilter', 'presupuestoFilterDropdown');
  ...
  function applyFilters() {
    filteredData = data.filter(row => {
      ...
      if (filters.nivel && row.NIVEL_DE_EDUCACION !== filters.nivel) return false;
      ...
      if (filters.presupuesto && row.TIPO_PRESUPUESTO !== filters.presupuesto) return false;
      return true;
    });
  ...
  function clearFilters() {
    ...
    ['amieTxt', 'provSel', 'cantSel', 'zonaSel', 'nivelSel', 'anioSel', 'regimenSel', 'presupuestoSel'].forEach(id => {
      const el = document.getElementById(id);
      if (!el) return;
      if (el.tagName === 'INPUT') el.value = '';
      else el.selectedIndex = 0;
    });
    ['anioFilterDropdown', 'regimenFilterDropdown', 'presupuestoFilterDropdown'].forEach(id => {
      const el = document.getElementById(id);
    ...
    return `
      <div class="popup-wrapper">
        ...
        <div class="popup-content">
          ...
          ${popupRow('Tipo de educación', row.TIPO_EDUCACION)}
          ${popupRow('Sostenimiento', row.SOSTENIMIENTO)}
          ${popupRow('Jurisdicción', row.JURISDICCION)}
          ${popupRow('Modalidad', row.MODALIDAD)}
          ${popupRow('Régimen', row.REGIMEN)}
          ${popupRow('Tipología', row.TIPOLOGIA)}
          ${popupRow('Zona INEC', row.ZONA_INEC)}
          ${popupRow('Nivel educativo', row.NIVEL_DE_EDUCACION)}
          ${popupRow('Año de dotación', row.AUX_ANIO_DOTACION)}
          ${popupRow('Tipo de presupuesto', row.TIPO_PRESUPUESTO)}
        </div>
      </div>
    `;
  }
  ...
  function updateStatistics() {
    ...
    const presupuestoOrder = ['BANCO MUNDIAL', 'Fiscal'];
    const budgetCounts = presupuestoOrder.map(tipo => ({
      tipo,
      count: filteredData.filter(row => row.TIPO_PRESUPUESTO === tipo).length
    }));

    const tbody = document.querySelector('#rubrosTable tbody');
    if (tbody) {
      tbody.innerHTML = budgetCounts.map(item => `
        <tr>
  ...
  function updateFilterStatus() {
    ...
    if (filters.presupuesto) activeFilters.push(`Tipo de presupuesto: ${filters.presupuesto}`);
  ...
  function updateMapSubtitle() {
    ...
    if (filters.presupuesto) {
      el.textContent = `VISUALIZACIÓN FILTRADA: ${filters.presupuesto}`;
    } else {
      el.textContent = 'VISUALIZACIÓN GENERAL DEL PRESUPUESTO';
    }
  }
