...
(function setupTableToggle(){
  ...
  function apply(hidden){
    ...
    setTimeout(()=> { if (typeof map !== 'undefined' && map && typeof map.invalidateSize==='function') map.invalidateSize(); }, 200);
  }
...
/* ---------- Toggle calculadora (dos botones sincronizados) ---------- */
function __setupCalcToggleInternal(){
  const panel = $("#transform-panel");
  const btnTop = $("#btn-toggle-calc");
  const btnInner = $("#btn-toggle-calc-inner");
  if (!panel || !btnTop || !btnInner) return;

  function apply(hidden){
    panel.classList.toggle("hidden", hidden);
    btnTop.textContent   = hidden ? "Mostrar calculadora" : "Ocultar calculadora";
    btnInner.textContent = hidden ? "Mostrar"             : "Ocultar";
    btnTop.setAttribute("aria-pressed", hidden ? "true" : "false");
    btnInner.setAttribute("aria-pressed", hidden ? "true" : "false");
    localStorage.setItem("calcHidden", hidden ? "1" : "0");
    setTimeout(()=> { if (typeof map !== 'undefined' && map && typeof map.invalidateSize==='function') map.invalidateSize(); }, 200);
  }

  const saved = localStorage.getItem("calcHidden") === "1";
  apply(saved);

  btnTop.addEventListener("click", () => apply(!panel.classList.contains("hidden")));
  btnInner.addEventListener("click", () => apply(!panel.classList.contains("hidden")));
}
window.addEventListener("DOMContentLoaded", __setupCalcToggleInternal);
/* ---------- Eventos e Init ---------- */
